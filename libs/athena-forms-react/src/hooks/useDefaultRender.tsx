...
export function useDefaultRender(opts: DefaultRenderOpts) {
  ...
  return useCallback((): React.ReactElement[] | undefined => {
    ...
    nodes.push(
      <Flex flexDirection={"row"} flexGap={"1rem"} key={currentFrame.name} flex={"1 auto"} flexGrow={1} flexShrink={1} flexStyles={{ flexWrap: "wrap" }}>
        {currentGroup.children.map((node: INode) => {
          ...
          return (
            <Flex key={node.name} flexDirection={"column"} flex={"1 auto"} flexGrow={1} flexShrink={1} flexStyles={{ minWidth: "45%", maxWidth: "45%" }}>
        ...
        {currentFrame.nodeGroups.length > 1 && (
          <Flex flexDirection={"row"} justifyContent={"space-between"} flexStyles={{ width: "100%" }}>
            <Flex flexGrow={1}>
